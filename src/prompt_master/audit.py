...
def audit(request: str, mode: str) -> list[AuditIssue]:
    """Find high-value missing context without inventing requirements."""
    text = " ".join(request.lower().split())
    issues: list[AuditIssue] = []

    if mode in {"coding", "sql", "data"} and not re.search(
        r"\b(code|query|schema|table|input|sample|example|file|data)\b", text
    ):
    ...
    if mode in {"writing", "creative"} and not re.search(
        r"\b(audience|tone|formal|casual|concise|short|long|length|style)\b", text
    ):
        issues.append(AuditIssue("PMA004", "Audience, tone, or length is unspecified; the result may vary significantly without it."))

    if mode == "image" and not re.search(
        r"\b(aspect|ratio|vertical|portrait|square|landscape|resolution|size|platform)\b", text
    ):
